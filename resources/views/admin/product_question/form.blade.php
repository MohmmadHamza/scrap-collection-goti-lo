    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="row mb-6">
                ...
                <div class="col-md">
                    <div class="card">
                        <h5 class="card-header">Product Question Create</h5>
                        <div class="card-body">
                            ...
                            <form id="inquiryForm" class="needs-validation" novalidate method="POST"
                                action="{{ isset($productQuestion) ? route('product-question.update', $productQuestion->id) : route('product-question.store') }}"
                                ...
                                <div class="row mb-6">
                                        ...
                                        <select class="form-select js-example-basic-single" id="category_id" name="category_id" required>
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ old('category_id', $productQuestion->subcategory->category_id ?? '') == $category->id ? 'selected' : '' }}>
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                            ...
                                            @if (isset($subcategories))
                                                @foreach ($subcategories as $subcategory)
                                                    <option value="{{ $subcategory->id }}"
                                                        {{ old('subcategory_id', $productQuestion->subcategory_id ?? '') == $subcategory->id ? 'selected' : '' }}>
                                                        {{ $subcategory->name }}
                                                    </option>
                                                @endforeach
                                            @endif
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" class="form-control" id="question_text" name="question_text"
                                            ...
                                            value="{{ old('question_text', $productQuestion->question_text ?? '') }}"
                                            required />
                                ...
                                <div class="row mb-6">
                                    <div class="col-md-5">
                                        ...
                                        <select class="form-select form-control-sm datatable-input" id="question_type"
                                            name="question_type" required>
                                            <option value="">Select Question Type</option>
                                            @foreach (['mcq', 'brand', 'text', 'image', 'video', 'document', 'long_text', 'select', 'numeric'] as $type)
                                                <option value="{{ $type }}"
                                                    {{ old('question_type', $productQuestion->question_type ?? '') == $type ? 'selected' : '' }}>
                                                    {{ ucfirst($type) }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" class="form-control" id="sort_order" name="sort_order"
                                            placeholder="Enter Question Text"
                                            value="{{ old('sort_order', $productQuestion->sort_order ?? '') }}" required />
                                    ...
                                    <div class="col-md-2 mt-6">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" id="statusToggle" name="status"
                                                value="active"
                                                {{ isset($productQuestion) && $productQuestion->status === 'active' ? 'checked' : '' }} />
                                ...
                                <div id="optionsContainer"
                                    class="col-md-6 {{ old('question_type', $productQuestion->question_type ?? '') == 'mcq' || old('question_type', $productQuestion->question_type ?? '') == 'select' ? '' : 'd-none' }}">
                                    <div class="col-md-10">
                                        <label class="form-label" for="options">Options</label>
                                        <div id="optionsWrapper">
                                            @if (isset($productQuestion) && !empty($productQuestion->options))
                                                @foreach (json_decode($productQuestion->options) as $option)
                                        ...
                                        <button type="button" class="btn btn-secondary" id="addOption">Add
                                            Option</button>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3 text-end">
                                    <button class="btn btn-primary mx-2" type="submit">
                                        {{ isset($productQuestion) ? 'Update' : 'Save' }}
                                    </button>
                                    <a href="{{ route('product-question.index') }}"
        ...
        if (categoryId) {
            ...
            fetch(`/get-subcategories/${categoryId}`)
                ...
                .then(data => {
                    ...
                    var selectedSubcategoryId = '{{ old("subcategory_id", $productQuestion->subcategory_id ?? '') }}';
